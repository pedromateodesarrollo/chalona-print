...
#   ./deploy-manager.sh --local        → /opt/print-server-hub/manager
...
case "${1:-}" in
  ...
  --produccion)
    ...
    tar -C dist -czf /tmp/print-server-manager.tgz .
    scp -q /tmp/print-server-manager.tgz "$DEPLOY_HOST:/tmp/"
    ssh "$DEPLOY_HOST" "rm -rf $INSTALL_DIR/manager && mkdir -p $INSTALL_DIR/manager && tar -C $INSTALL_DIR/manager -xzf /tmp/print-server-manager.tgz && rm /tmp/print-server-manager.tgz && ls $INSTALL_DIR/manager"
    ;;
  *)
